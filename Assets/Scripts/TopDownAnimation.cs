...
void Update()
    {
        ...
        if (clickMove != null)
        {
            if(clickMove.YMag != 0 || clickMove.XMag != 0)
            {
                idleOn = false;
                animateString = "Walk";
                //gameObject.transform.rotation.eulerAngles.z = Mathf.Atan2((screenPos.y - transform.position.y), 
                //    (screenPos.x - transform.position.x)) * Mathf.Rad2Deg;

                var deltaX = clickMove.position.x - transform.position.x;
                var deltaY = clickMove.position.y - transform.position.y;
                float deg = (float)(Mathf.Atan2(clickMove.YMag, clickMove.XMag) * Mathf.Rad2Deg);
                var d = Mathf.LerpUnclamped(0, 180, clickMove.XMag);
                d = clickMove.YMag > 0 ? d : -d;
